...
from train_test import prepare_trte_data, test_epoch
...
def cal_feat_imp(data_folder, model_folder, view_list, num_class):
    ...
    for i in range(len(featname_list)):
        ...
        for j in range(dim_list[i]):
            ...
            te_prob = test_epoch(data_trte_list, adj_te_list, trte_idx["te"], model_dict)
            f1_tmp = f1_score(labels_trte[trte_idx["te"]], te_prob.argmax(1), average='weighted')
...
def summarize_imp_feat(featimp_list_list, topn=200):
